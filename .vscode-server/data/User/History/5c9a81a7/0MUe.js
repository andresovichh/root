...
const updateTutorial = async (req, res) => {
    try{
        ...
        const tutorial =  {id, Title, Video_Url, Description, Published_Status, Deleted_At};
        const connection = await getConnection();
        const result = await connection.query("UPDATE tutorial_list SET ? WHERE id = ?", tutorial);
        res.json(result);
    } catch (error) {
            res.status(500);
            res.send(error.message);
